...
double centralCoordinate = pixelQuantity / 2;
...
public:
	...
	void DrawText(double x, double y, int num);
...
void TimerFunction(int value)
{
	...
	glutPostRedisplay();
	glutTimerFunc(10, TimerFunction, 100);
}
...
void init(int argc, char **argv)
{
	...
	text.CreateTextFont("Courier New", 20);
...
{
	cout << "If you want start game - press <<s>>" << endl;
...
void Text::CreateTextFont(char *fontName, int size)
{
	...
	HFONT Font = CreateFont(FontSize, 0, 0, 0, 21, 0, 0, 0, 204, 3, 2, 1, 34, fontName);
...
void Text::DrawText(double x, double y, int num, int currentPlayer)
{
	...
	{
		figure = num / pow(10.0, (NumberLength(num) - pos - 1));
...
void Text::DrawText(double x, double y, int num)
{
	glColor3f(.2, .7, .7);
	char number[10];
	int figure;
...
leftmostPointAngle = -180 - partAngle*1.01;
double mnojX = (target.doubleRadius*1.12),
mnojY = (target.doubleRadius*1.12);

void DrawScoresOnTarget(int sector, int scores)
{
	double x = cos(leftmostPointAngle + partAngle * sector*PI / 180) * mnojX + centralCoordinate - 20;
	double y = sin(leftmostPointAngle + partAngle * sector*PI / 180) * mnojY + centralCoordinate - 15;
	text.DrawText(x, y, scores);
}

void DrawScoresAboutSector()
{
	DrawScoresOnTarget(1, 11);
	DrawScoresOnTarget(2, 8);
	DrawScoresOnTarget(3, 16);
	DrawScoresOnTarget(4, 7);
	DrawScoresOnTarget(5, 19);
	DrawScoresOnTarget(6, 3);
	DrawScoresOnTarget(7, 17);
	DrawScoresOnTarget(8, 2);
	DrawScoresOnTarget(9, 15);
	DrawScoresOnTarget(10, 10);
	DrawScoresOnTarget(11, 6);
	DrawScoresOnTarget(12, 13);
	DrawScoresOnTarget(13, 4);
	DrawScoresOnTarget(14, 18);
	DrawScoresOnTarget(15, 1);
	DrawScoresOnTarget(16, 20);
	DrawScoresOnTarget(17, 5);
	DrawScoresOnTarget(18, 12);
	DrawScoresOnTarget(19, 9);
	DrawScoresOnTarget(20, 14);
}
					...
					if (lastDefinitePoint == maxShot)
					{
						...
						if (player[currentPlayer].scores == 0 && gameStarted)
						{
							cout << endl << "WOW! WE HAVE A WINNER!!! " << endl << "The " << " Player" <<  currentPlayer + 1 <<  " is won this match! Congratulations!"
								<< endl << "Would you want repeat game?(y/n)" << endl;
						...
						if (answer == 'n')
						{
							exitGame = true;
							gameStarted = false;
							cout << endl << "If you want start game - press <<s>>";
							goto end;
						}
						else if (answer == 'y')
						{
